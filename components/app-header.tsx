    ...
    <header className="border-b border-cyan-500/20 bg-slate-950/30 backdrop-blur-xl">
      <div className="flex h-14 sm:h-16 items-center justify-between px-3 sm:px-4 md:px-6">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
          <SidebarTrigger className="text-cyan-400 hover:text-cyan-300 hover:bg-cyan-500/10 h-8 w-8 sm:h-10 sm:w-10" />
              ...
              <div key={crumb.href} className="flex items-center min-w-0">
                ...
                <Link
                  ...
                  className={`hover:text-cyan-300 transition-colors truncate ${
                    index === breadcrumbs.length - 1 ? "text-cyan-400 font-medium" : "text-slate-400"
                  }`}
                >
        ...
        <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
          {!isSupabaseConfigured ? (
            <div className="text-yellow-400 text-xs sm:text-sm font-medium hidden sm:block">Development Mode</div>
          ) : user ? (
            <DropdownMenu>
            ...
            <Button
              variant="ghost"
              size="sm"
              className="text-cyan-400 hover:text-cyan-300 hover:bg-cyan-500/10 text-xs sm:text-sm"
            >
              Guest User
            </Button>
          )}
